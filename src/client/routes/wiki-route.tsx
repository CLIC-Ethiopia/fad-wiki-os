...
function computeScatteredLayout(
  ...
  for (let i = 0; i < displayed.length; i++) {
    ...
    nodes.push({
      ...
      size: Math.max(3.5, Math.min(10, 3.5 + Math.sqrt(n.backlinkCount) * 1.5)),
      isCenter: false,
    });
  }
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = 1; i < nodes.length; i++) {
        ...
        ctx.lineTo(n.x, n.y);
        ctx.strokeStyle = hoveredIdx === i ? "rgba(255,255,255,0.3)" : "rgba(255,255,255,0.12)";
        ctx.lineWidth = hoveredIdx === i ? 1.5 : 0.8;
        ctx.stroke();
      }
      ...
      for (let i = nodes.length - 1; i >= 0; i--) {
        ...
        if (isHovered) {
          ctx.beginPath();
          ctx.arc(n.x, n.y, drawSize + 5, 0, Math.PI * 2);
        ...
        if (n.isCenter || isHovered) {
          ...
          ctx.fillStyle = n.isCenter ? "rgba(255,255,255,0.95)" : "rgba(255,255,255,0.7)";
