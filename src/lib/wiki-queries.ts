...
export async function getDerivedData(deps: WikiQueryDependencies): Promise<DerivedData> {
  ...
  const topConnectedRows = db
    .prepare(`
      SELECT
        b.target_slug AS slug,
        COALESCE(p.title, MIN(b.target_raw)) AS title,
        COALESCE(p.summary, '') AS summary,
        CAST(SUM(b.occurrence_count) AS INTEGER) AS backlinkCount,
        COALESCE(p.word_count, 0) AS wordCount,
        COALESCE(p.modified_at, 0) AS modifiedAt,
        COALESCE(p.file, '') AS file
      FROM backlinks b
      LEFT JOIN pages p ON p.slug = b.target_slug
      GROUP BY b.target_slug
      ORDER BY backlinkCount DESC, title ASC
      LIMIT 6
    `)
    .all() as PageSummary[];

  const topConnected = topConnectedRows.map(row => ({
    ...row,
    title: row.slug.startsWith('tag-') ? '#' + row.slug.replace('tag-', '') : row.title
  }));
  ...
  const latestNote = [...pageSummaries]
    .sort((a, b) => b.modifiedAt - a.modifiedAt)
...
export async function getGraphData(deps: WikiQueryDependencies): Promise<GraphData> {
  ...
  // Create virtual nodes for missing targets (e.g. tags or uncreated pages)
  const existingSlugs = new Set(nodes.map(n => n.slug));
  for (const edge of edges) {
    if (!existingSlugs.has(edge.target)) {
      nodes.push({
        slug: edge.target,
        title: edge.target.startsWith('tag-') ? '#' + edge.target.replace('tag-', '') : edge.target,
        summary: '',
        backlinkCount: 1, // Will be inaccurate but works for visualization
        wordCount: 0,
        categoryNamesJson: '[]'
      });
      existingSlugs.add(edge.target);
    }
  }
