...
import { tutorRoutes } from "./tutor/tutor-routes";
...
export async function buildServer({
  ...
  // Dynamic import so it does not fail if web-clipper dependencies are missing at build time
  app.get("/api/directories", async (_request, reply) => {
    try {
      const { getVaultDirectories } = await import("./web-clipper");
      return await getVaultDirectories();
    } catch (error) {
      return replyForWikiError(error, reply, "Failed to get directories", 500);
    }
  });

  app.post<{ Body: { url: string } }>("/api/clip/parse", async (request, reply) => {
    try {
      const { url } = request.body;
      const { parseWebClipperUrl } = await import("./web-clipper");
      return await parseWebClipperUrl(url);
    } catch (error) {
      return replyForWikiError(error, reply, "Failed to parse URL", 500);
    }
  });

  app.post<{ Body: { title: string, markdown: string, folder: string } }>("/api/clip/save", async (request, reply) => {
    try {
      const { title, markdown, folder } = request.body;
      const { saveWebClipperClip } = await import("./web-clipper.js");
      const filePath = await saveWebClipperClip(title, markdown, folder);
      return { ok: true, file: filePath };
    } catch (error) {
      return replyForWikiError(error, reply, "Failed to save clip", 500);
    }
  });

  app.get("/api/recommender/nodes", async (_request, reply) => {
    try {
      const { getVaultNodes } = await import("./recommender.js");
      return await getVaultNodes();
    } catch (error) {
      return replyForWikiError(error, reply, "Failed to get nodes", 500);
    }
  });

  app.post<{ Body: { apiKey: string, targetPath: string, customPrompt?: string } }>("/api/recommend", async (request, reply) => {
    try {
      const { apiKey, targetPath, customPrompt } = request.body;
      const { generateRecommendations } = await import("./recommender.js");
      const recommendations = await generateRecommendations(apiKey, targetPath, customPrompt);
      return { ok: true, recommendations };
    } catch (error) {
      return replyForWikiError(error, reply, "Failed to generate recommendations", 500);
    }
  });

  app.post<{ Body: { apiKey: string, message: string, history?: any[] } }>("/api/chat", async (request, reply) => {
    try {
      const { apiKey, message, history } = request.body;
      const { handleChatQuery } = await import("./chat.js");
      const response = await handleChatQuery(apiKey, message, history || []);
      return { ok: true, response };
    } catch (error) {
      return replyForWikiError(error, reply, "Failed to generate chat response", 500);
    }
  });

  await app.register(tutorRoutes, { prefix: "/api/tutor" });
